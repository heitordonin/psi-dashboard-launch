...
-- 2. Adicionar constraints para prevenir múltiplas assinaturas ativas
CREATE UNIQUE INDEX CONCURRENTLY IF NOT EXISTS idx_user_subscriptions_active_user 
ON public.user_subscriptions (user_id) 
WHERE status = 'active';

-- 3. Função de cleanup automático das assinaturas antigas
...
-- 4. Atualizar função de force sync para usar upsert
...
-- 5. Criar constraint para garantir apenas uma assinatura ativa por usuário
-- (será executado após o índice ser criado)
DO $$
BEGIN
  ...
  IF NOT EXISTS (
    ...
    -- Primeiro limpar duplicatas existentes se houver
    WITH duplicates AS (
      SELECT 
        user_id,
        id,
