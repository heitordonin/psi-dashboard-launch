-- Security Fix Implementation: Comprehensive Security Hardening (Fixed)
...
-- =====================================================================================
-- PHASE 2: BUSINESS DATA PROTECTION - Fix existing policies properly
-- =====================================================================================

-- Remove public access to subscription_plans table
DROP POLICY IF EXISTS "Anyone can view active subscription plans" ON public.subscription_plans;

-- Update existing policy for subscription_plans (don't create new one if exists)
DO $$ 
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_policies 
        WHERE tablename = 'subscription_plans' 
        AND policyname = 'Authenticated users can view subscription_plans'
    ) THEN
...
-- Remove overly broad public access to banks table but keep the existing policy name
DROP POLICY IF EXISTS "Anyone can view active banks" ON public.banks;

-- The "Authenticated users can view banks" policy already exists, so we keep it
...
-- Add secure policy for admin_audit_log to prevent unauthorized deletion
DO $$ 
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_policies 
...
COMMENT ON POLICY "Authenticated users can view subscription_plans" ON public.subscription_plans IS 'Restricts plan access to authenticated users only';
