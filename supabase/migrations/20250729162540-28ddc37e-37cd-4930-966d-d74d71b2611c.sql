...
AS $function$
  ...
  SELECT 
    COALESCE(SUM(cmr.price_monthly * cmr.active_subs), 0) as total_mrr,
    ...
    CASE 
      WHEN pmr.prev_total > 0 THEN 
        ROUND((COALESCE(SUM(cmr.price_monthly * cmr.active_subs), 0) - pmr.prev_total) / pmr.prev_total * 100, 2)
      ELSE 0 
...
AS $function$
  WITH conversion_data AS (
    SELECT 
      -- Usuários que começaram com free e depois mudaram para pago
      COUNT(*) FILTER (
        WHERE EXISTS (
      ...
      -- Total de usuários que começaram com free
      COUNT(*) FILTER (
        WHERE sp.slug = 'free'
      ) as total_free_users,
      
      -- Conversões de gestao para psi_regular
      COUNT(*) FILTER (
...
AS $function$
  ...
  SELECT 
    md.month,
    COALESCE(
      LAG(SUM(CASE WHEN sc.change_type = 'new' THEN sc.price_monthly ELSE 0 END), 1) 
      OVER (ORDER BY md.month), 0
    ) + 
    COALESCE(SUM(CASE WHEN sc.change_type = 'new' THEN sc.price_monthly ELSE 0 END), 0) + 
    COALESCE(SUM(CASE WHEN sc.change_type = 'churned' THEN sc.price_monthly ELSE 0 END), 0) as mrr,
    
    COALESCE(SUM(CASE WHEN sc.change_type = 'new' THEN sc.price_monthly ELSE 0 END), 0) as new_mrr,
    COALESCE(ABS(SUM(CASE WHEN sc.change_type = 'churned' THEN sc.price_monthly ELSE 0 END)), 0) as churned_mrr,
