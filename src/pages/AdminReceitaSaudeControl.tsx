...
const AdminReceitaSaudeControl = () => {
  ...
  const { data: patients = [] } = useQuery({
    queryKey: ['admin-patients', selectedUserId, showAllUsers],
    queryFn: async () => {
      if (showAllUsers) {
        // Get patients from all Psi Regular users
        ...
        // Get patients from specific user
        const { data, error } = await supabase
          .from('patients')
          .select('*')
    ...
    enabled: showAllUsers || !!selectedUserId
  });

  const { data: payments = [], isLoading: paymentsLoading } = useQuery({
    queryKey: ['admin-payments-receita-saude', selectedUserId, showAllUsers],
    queryFn: async () => {
      if (showAllUsers) {
        // Get payments from all Psi Regular users
        const { data: planData } = await supabase
          .from('subscription_plans')
          .select('id')
          .eq('slug', 'psi_regular')
        ...
        // Get payments from specific user
        const { data, error } = await supabase
          .from('payments')
          .select(`
            *,
  ...
  const handleUserFilterChange = (userId: string | null, showAll: boolean) => {
  ...
  const handleReceiptToggle = (paymentId: string, currentStatus: boolean) => {
    ...
    queryClient.setQueryData(['admin-payments-receita-saude', selectedUserId, showAllUsers], (oldData: any) => {
          ...
          <div className="min-h-screen bg-gray-50">
            ...
            <div style={{ backgroundColor: '#002472' }} className="border-b px-4 py-4">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <h1 className="text-xl font-semibold" style={{ color: '#ffffff' }}>Admin - Controle Receita Saúde</h1>
                  <p className="text-sm" style={{ color: '#03f6f9' }}>Gerenciamento de recibos emitidos no Receita Saúde dos usuários Psi Regular</p>
                </div>
              </div>
            </div>
            ...
            <div className="container mx-auto px-4 py-6 space-y-6">
              {/* User Filter */}
              <UserFilter onFilterChange={handleUserFilterChange} />
                    ...
                    <div className="text-center py-8">
                      ...
                      <p className="text-gray-600 mb-2">
                        {searchTerm || filters.patientId || filters.startDate || filters.endDate || filters.minAmount || filters.maxAmount || filters.receiptStatus
                          ...
                          : 'Nenhum pagamento pago encontrado para os usuários selecionados'
                        }
                      </p>
                      {!showAllUsers && !selectedUserId && (
                        <p className="text-gray-500 text-sm">Selecione um usuário específico ou marque para mostrar todos os usuários</p>
                      )}
                    </div>
                  ) : (
                            ...
                            <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                              ...
                              <div className="space-y-3">
                                ...
                                <div className="border-t pt-3">
                                  {receiptEligibility.isEligible ? (
                                    <div className="flex items-center justify-between">
                                      <span className="font-medium text-gray-500 text-sm">Receita Saúde:</span>
...
export default AdminReceitaSaudeControl;